...
Members never log in. `/` is the in-product gym home for **Bloc Shop Chabanel**.
...
3. Friendly URLs: `/m/bloc-shop/lat-pulldown`.
...
Keyed by gym slug (`bloc-shop`). No member account.

| Key | Contents |
|-----|----------|
| `bloc-shop-workout:v1:<slug>` | In-progress session JSON |
| `bloc-shop-workout-history:v1:<slug>` | Array of saved workouts (newest first) |
